...
backtestStartDate = as.Date("2013-01-02") #Starting date for the backtest

symbolLst<-c("SPXL","TMF")
title<-paste(symbolLst[1],symbolLst[2])
...
getSymbols("SPY", env = symbolData, src = "yahoo", from = backtestStartDate)
...
stockPair <- list(
  a = coredata(Ad(eval(parse(text=paste("symbolData$\"",symbolLst[1],"\"",sep="")))))   #Stock A
  ,b = coredata(Ad(eval(parse(text=paste("symbolData$\"",symbolLst[2],"\"",sep=""))))) #Stock B
  ,hedgeRatio = 0
  ,name=title)

regression <- lm(stockPair$a ~ stockPair$b + 0)
stockPair$hedgeRatio = coef(regression)[1]

print(stockPair$hedgeRatio)
...
simulateTrading <- function(stockPair){
  ...
  dev.new()
  par(mfrow=c(2,1))
  plot(movingAvg,col="red",ylab="Spread",type='l',lty=2, ylim=c(-40,40))
...
indexRet <- Delt(Ad(eval(parse(text=paste("symbolData$\"",symbolLst[1],"\"",sep="")))),k=1,type="arithmetic") #Daily returns
